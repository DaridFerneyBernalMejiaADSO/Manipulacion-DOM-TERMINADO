        ...
        for(let val in data[dataInput.scale]){
            if(data[dataInput.scale][val][0] <= dataInput.nota && data[dataInput.scale][val][1] >= dataInput.nota){
                ...
                myTabla.insertAdjacentHTML("beforeend",`
                    ...
                    <tr>
                        <td colspan="5">${(val == 0) ? "Querido usuario se gano un viaje a <b>venezuela</b>" : (val == 1) ? "Querido usuario se gano unas voletas para Cine Colombia para ver la pelicula de <b>Petro con maduro</b>" : (val ==2) ? "Querido usuario se ganaste la rifa de <b> Doña blanca</b>": (val ==3) ? "Querido usuario se ganaste medio pollo <b>(un pernil)</b>":(val ==4) ? "Querido usuario se ganaste un  <b>iphone 15 pro MAX</b>": (val ==5) ? "te ganaste un raspa y gana <b>solo dice `sigue intentando`</b>":""}</td>
                    </tr>
                `);
            }   
        }
